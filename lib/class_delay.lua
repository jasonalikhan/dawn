...
--- constructor
function machine_delay:new(machine_num)
  local o = {}
  self.__index = self
  setmetatable(o, self)
...
-- grid_key
--
--- process grid button presses
--

function machine_delay:grid_key(x, y, z)
  self.grid_dirty = true
end

-- grid_redraw_handler
--
-- refresh grid leds, driven by fast metro
--

function machine_delay:grid_redraw(g)
  if self.grid_dirty == true then
    g:all(0)

    -- current selected instrument
    if self.machine_num <= 4 then
      g:led(12+self.machine_num, 8, 15)
    end
